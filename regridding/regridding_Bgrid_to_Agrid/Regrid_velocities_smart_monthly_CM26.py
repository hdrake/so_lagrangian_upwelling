...
path2files = '/archive/'+user+'/'+model+'/'+exp+'/monthly/for_CMS_SO/'

# Path to save velocity files
path2save   = '/archive/'+user+'/'+model+'/'+exp+'/monthly/for_CMS_SO_Edited/'
...
for file in files:
    ...
    if ((year == 181) and (month == 1) and (day == 15)):
    ...
    # save u:
    out_filename_uvw = path2save+'ocean.'+str(year).zfill(4)+str(month).zfill(2)+'15'+'.uvw.nc'
