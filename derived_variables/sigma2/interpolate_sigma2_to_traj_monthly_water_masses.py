...
total_sim_time = int(20*12*2592000)
...
for p in range(npt):
    ...
    days_between_particle_release_and_start_date[p] = sum(gcal2jd(release_years[p],release_months[p],release_days[p]))+release_seconds_dec[p]-sum(gcal2jd(181,1,07))

days_after_release = np.zeros(ntime)
for t in range(0,ntime):
    days_after_release[t] = float(times[t])/86400

# Calculate julian date of each particle at each time step, where gcal2jd(181,1,7) = 1787175.5
julian_dates = np.zeros((npt,ntime))
for p in range(npt):
    for t in range(ntime):
        julian_dates[p,t] = np.mod(days_after_release[t]+days_between_particle_release_and_start_date[p],total_sim_time) + 1787175.5
...
if upwell_flag:
    ...
    for pt in range(0,npt):
        if np.size(np.where(np.logical_and((traj_depths_OG[pt,:] < 1000),(traj_depths_OG[pt,:] > 0)))):
            ...
            julian_dates[pt,upwell_time[pt]:] = np.nan

    # Only consider particles that upwell
    # Meta data
    upwell1000_inds = np.where(upwell_time)
    ...
    # Trajectory data
    traj_lats = traj_lats_up[upwell1000_inds,:]
    traj_lats_OG = traj_lats[upwell1000_inds,:]

    traj_lons = traj_lons_up[upwell1000_inds,:]
    traj_lons_OG = traj_lons[upwell1000_inds,:]

    traj_depths = traj_depths_up[upwell1000_inds,:]
    traj_depths_OG = traj_depths[upwell1000_inds,:]

    sigma2 = sigma2[upwell1000_inds,:]
    julian_dates = julian_dates[upwell1000_inds]
...
for yeari in range(startyear,startyear+20):
    for monthi in range(1,13):
        print 'For year '+str(yeari)+' and month '+str(monthi)
...
if ~upwell_flag:
    ...
    trajs = np.zeros((npt,ntime,5))
    trajs[:,:,0] = traj_lons
    trajs[:,:,1] = traj_lats
    trajs[:,:,2] = traj_depths
    trajs[:,:,3] = sigma2
    trajs[:,:,4] = julian_dates
    ...
    trajs_meta[:,4] = upwell_time
    trajs_meta[:,5] = stuck_time
