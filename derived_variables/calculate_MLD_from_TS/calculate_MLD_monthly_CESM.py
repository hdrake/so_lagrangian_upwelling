...
path2sigma = '/archive/hfd/'+model+'/'+exp+'/monthly/sigma0/'
path2MLD = '/archive/hfd/'+model+'/'+exp+'/monthly/MLD/'
htfilename = '/archive/Carolina.Dufour/Mosaic/'+model+'/ocean.static.nc'

# Calculate ht
nc_sigma0 = nc.Dataset(path2sigma+'nest_1_00670115000000d.nc')
sigma0_mask = nc_sigma0.variables['sigma0'][0,:,:,:].mask
st_ocean = nc_sigma0.variables['st_ocean'][:]
xt_ocean = nc_sigma0.variables['xt_ocean'][:]
yt_ocean = nc_sigma0.variables['yt_ocean'][:]
ht = st_ocean[np.maximum(np.zeros((np.size(yt_ocean),np.size(xt_ocean))).astype(int),np.argmax(sigma0_mask, axis = 0)-1)]
nc_sigma0.close()
...
for file in files:
    ...
    MLD = np.minimum(st_ocean[np.argmax(sigma0 > MLD_sigma0, axis=0)],ht)
