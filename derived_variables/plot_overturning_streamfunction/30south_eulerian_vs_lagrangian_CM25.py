...
rho_0 = 1035.
griddir = '/archive/Carolina.Dufour/Mosaic/CM2.5/CM2.5'
grid_file = griddir+'_grid_spec.nc'
gridFile = nc.Dataset(grid_file)
dxu_cm21 = gridFile.variables['dxu'][...]
gridFile.close()
CM21_file = '/archive/hfd/CM2.5/CM2.5_A_Control-1860_Y03-MBLING-BLING/5day/for_CMS_SO_Edited/ocean.01810107.uvw.nc'
...
plt.gca().invert_yaxis()
plt.suptitle('Zonal sum of southward transport at 30 south, derived from CM2-1deg velocities in red \nand same velocities interpolated onto the CM2.6 grid in blue. The sum between \nthe cyan lines is '+str(np.sum(transport_cm26_neg[27:40,:]))+' for interpolated fields and '+str(np.sum(transport_cm21_neg[27:40,:]))+' for '+model+' fields.')
plt.ylabel('Depth (m)')
...
plt.figure(3)
plt.clf()
transport_cm21_pos = np.copy(transport_cm21)
transport_cm26_pos = np.copy(transport_cm26)
transport_cm21_pos = np.ma.masked_array(transport_cm21_pos,mask = cm21_mask)
transport_cm26_pos = np.ma.masked_array(transport_cm26_pos,mask = cm26_mask)
transport_cm21_pos[transport_cm21_pos < 0] = 0
transport_cm26_pos[transport_cm26_pos < 0] = 0
plt.plot(np.sum(transport_cm21_pos,axis = 1),st_ocean21,'r')
plt.plot(np.sum(transport_cm26_pos,axis = 1),st_ocean26,'b')
plt.plot([-5,4],[1000,1000],'c--')
plt.plot([-5,4],[3500,3500],'c--')
plt.plot([0,0],[st_ocean26[0],st_ocean26[-1]],'k')
plt.gca().invert_yaxis()
plt.suptitle('Zonal sum of northward transport at 30 south, derived from CM2-1deg velocities in red \nand same velocities interpolated onto the CM2.6 grid in blue. The sum between \nthe cyan lines is '+str(np.sum(transport_cm26_neg[27:40,:]))+' for interpolated fields and '+str(np.sum(transport_cm21_neg[27:40,:]))+' for '+model+' fields.')
plt.ylabel('Depth (m)')
plt.xlabel('Zonal sum of northward transport on each model depth level (Sv)')
plt.draw()
plt.show()
#plt.savefig('/work/hfd/figures/southward_velocity_interpolation.png')
