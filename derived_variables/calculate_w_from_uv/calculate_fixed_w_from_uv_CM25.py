...
path2ice = '/archive/wga/Siena/siena_201303_fix_rds-whit/CM2.5_A_Control-1860_Y03-MBLING-BLING/pp/ice_daily/ts/daily/5yr/'
path2files = '/archive/akm/CM2.5/CM2.5_A_Control-1860_Y03-MBLING-BLING/history/'
path2save = '/archive/hfd/CM2.5/CM2.5_A_Control-1860_Y03-MBLING-BLING/5day/for_CMS_SO/'
...
last_year = 200
boundary_north = -29.5
index_north = np.where(lat_t[:,0]<boundary_north)[0][-1]+1

# Sw_ocean
data = nc.Dataset('/archive/wga/Siena/siena_201303_fix_rds-whit/CM2.5_A_Control-1860_Y03-MBLING-BLING_rerun/history/02000101.ocean_bgc_physics_field_w.nc')
...
dxt = np.zeros((50,n_yt,1440))
dyt = np.zeros((50,n_yt,1440))
dxu = np.zeros((50,n_yt,1440))
dyu = np.zeros((50,n_yt,1440))
...
# Define arrays
up_flux = np.zeros((50,n_yt,1440))
online_up_flux = np.zeros((50,n_yt,1440))
...
for file in files:
    ...
    for countu5day in range(0,73):
        icecount5day = (countu5day+countfiles*73)%(73*5)
        ...
        file_name_ice = 'ice_daily.'+str((int(year-1)/5)*5+1).zfill(4)+'0101-'+str((int(year-1)/5)*5+5).zfill(4)+'1231.'
        ...
        SSH_data = nc.Dataset(path2ice+file_name_ice+'SSH.nc')
        EXT_data = nc.Dataset(path2ice+file_name_ice+'EXT.nc')
        ...
        if (rho_dzt_t1 < ice_time[0]):
            ...
            file_name_ice_before = 'ice_daily.'+str((int(year-1)/5)*5-4).zfill(4)+'0101-'+str((int(year-1)/5)*5).zfill(4)+'1231.'
            SSH_data_before = nc.Dataset(path2ice+file_name_ice_before+'SSH.nc')
            EXT_data_before = nc.Dataset(path2ice+file_name_ice_before+'EXT.nc')
            ...
            # SSH tendency terms from ice
            file_name_ice_after = 'ice_daily.'+str((int(year-1)/5)*5+6).zfill(4)+'0101-'+str((int(year-1)/5)*5+10).zfill(4)+'1231.'
            SSH_data_after = nc.Dataset(path2ice+file_name_ice_after+'SSH.nc')
            EXT_data_after = nc.Dataset(path2ice+file_name_ice_after+'EXT.nc')
